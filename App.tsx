                ...
                {shoppingList.length > 0 ? (
                  <div className="bg-white dark:bg-[#1e293b] rounded-2xl sm:rounded-[3.5rem] shadow-sm border border-gray-100 dark:border-gray-800 overflow-hidden">
                    {shoppingList.map((item, idx) => {
                      ...
                      return (
                        <div key={item.id} className={`flex flex-col sm:flex-row items-center justify-between p-4 sm:p-8 ${idx !== shoppingList.length - 1 ? 'border-b border-gray-50' : ''} hover:bg-[#f8fafc] group gap-4`}>
                          <div className="flex items-center space-x-4 w-full"><div className="min-w-0"><p className="text-lg sm:text-2xl font-[900] truncate text-gray-900 dark:text-gray-100">{item.productName}</p><div className="flex items-center mt-1 space-x-2">{storeLogo && <div className="w-5 h-5 bg-white rounded-md p-0.5 border border-gray-100 flex items-center justify-center"><img src={storeLogo} alt="" className="w-full h-full object-contain" /></div>}<p className="text-[10px] font-black text-gray-400 uppercase tracking-widest">{item.originalStore}</p></div></div></div>
